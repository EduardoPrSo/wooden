    ...
    const displayProducts = productsData.map((product, index) => (
        <div key={index} className={styles.productDiv}>
            {windowWidth > 900 ? index % 2 === 0 ? (
                <>
                    <div className={styles.productImage} style={{transform: scrollY > (500+(index*(windowWidth/5))) ? "translateX(0)" : "translateX(-200%)", transition: '1s' }}>
                        <div className={styles.imageCarousel}>
                            <img src={product.images[0]} style={{ width: '25vw', height: '70vh', boxShadow: '5px 5px 10px #808080'}} />
                        </div>
                    </div>
                    <div className={styles.productInfo} style={{width: '30vw', height: '55vh', marginTop: '10%', transform: scrollY > (500+(index*(windowWidth/5))) ? "translateX(0)" : "translateX(+200%)", transition: '1s'  }}>
